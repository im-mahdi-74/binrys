...
def xau_cnn_lstm():
  ...
  model.load_state_dict(torch.load(r'xau/xauusd_cnnlstm_v5.pth',weights_only=True))
  return model
...
def xau_lstm():
    ...
    lstm_model.load_state_dict(torch.load(r'xau/xauusd_lstm_v5.pth' ,weights_only=True))
    return lstm_model
...
def final_model():
    ...
    model.load_state_dict(torch.load(r'xau/xau_metamodel.pth',weights_only=True))
...
def xau_scale():
    ...
    with open(r'xau.pkl', 'rb') as f:
        return pickle.load(f)




model_xau_cnn_lstm = xau_cnn_lstm()
model_xau_lstm = xau_lstm()
...
def run(symol , scale, models , xau_final_model):
    ...
    while True:
        ...
        # if (datetime.datetime.now().second + 1) >= 55:
            ...
            # try:
                ...
                final_pred = (final( xau_final_model ,main_tensor )).detach().item()
...
xau_scaler = xau_scale()
xau_final_model = final_model()
...
th.Thread(target= run , args=(  symol , xau_scaler , [model_xau_lstm , model_xau_cnn_lstm  ] , xau_final_model )).start()
